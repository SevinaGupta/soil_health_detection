...
from app.models import User, MositureData, PhData
...
def input_page():
    if request.method == 'POST':
        mosi_id = request.form.get('mosi_id')
        mosi_per = request.form.get('mosi_per')
        densy = request.form.get('densy')
        if mosi_id:      # not none
            if mosi_per: # just some validation
                msgObj = MositureData(id=mosi_id,mositure=mosi_per,density=densy)   #add data to model object
                db.session.add(msgObj)    #save data in database
                db.session.commit()       #update database
                #prediction logic
                flash('we have save ur data,predidition result will be avilable shortly','success')
            else:
                flash('message cannot be provided','danger')
        else:
            flash('message not provided','please fill some data')
    return render_template('input.html',title="Input data")

@app.route('/inputph',methods=['GET', 'POST'])
def input2_page():
    if request.method == 'POST':
        mosi_id = request.form.get('mosi_id')
        ...
        densy = request.form.get('densy')
        if mosi_id:      # not none
            if ph: # just some validation
                msgObj = PhData(id=mosi_id,pH=ph,density=densy)   #add data to model object
                db.session.add(msgObj)    #save data in database
                db.session.commit()       #update database
                #prediction logic
                flash('we have save ur data,predidition result will be avilable shortly','success')
            ...
            flash('message not provided','please fill some data')
    return render_template('inputph.html',title="Input data")
